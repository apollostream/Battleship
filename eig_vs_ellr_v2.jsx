...
const runGame = (strategy, trueHypIdx, threshold, initW) => {
  let ws = [...initW], nq = 0;
...
export default function App() {
  const DESIGNED_W = [0.45, 0.45, 0.05, 0.05];
  const UNIFORM_W  = [0.25, 0.25, 0.25, 0.25];

  const [priors,   setPriors]   = useState(DESIGNED_W);
  const [weights,  setWeights]  = useState(DESIGNED_W);
  ...
  // ── Prior editing ────────────────────────────────────────────────
  const setPriorAt = useCallback((idx, raw) => {
    const v = Math.max(0.01, Math.min(0.97, raw));
    setPriors(prev => {
      const rest = prev.reduce((s, w, i) => i === idx ? s : s + w, 0);
      const scale = rest > 1e-9 ? (1 - v) / rest : 0;
      const next = prev.map((w, i) => i === idx ? v : w * scale);
      // normalise for floating-point safety
      const Z = next.reduce((a,b) => a+b, 0);
      const normed = next.map(w => w/Z);
      setWeights(normed);
      setHistory([]);
      setSimStale(true);
      return normed;
  ...
  const reset = () => { setWeights(priors); setHistory([]); setSimRes(null); setSimStale(false); };
      ...
      for (let g=0; g<N_SIM; g++) {
        for (const s of ["eig","ellr"]) {
          for (let hi=0; hi<HYPS.length; hi++) {
            const r = runGame(s, hi, threshold, priors);
      ...
      <div style={S.row}>
        ...
        <div style={{ ...S.card, minWidth:290 }}>
          ...
          <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:6 }}>
            <div style={S.h3}>Hypotheses — click row to set ground truth</div>
            <div style={{ display:"flex", gap:4 }}>
              <button style={{ ...S.btn("#1e293b"), padding:"2px 8px", fontSize:10 }}
                      onClick={() => applyPreset(DESIGNED_W)}>Designed</button>
              <button style={{ ...S.btn("#64748b"), padding:"2px 8px", fontSize:10 }}
                      onClick={() => applyPreset(UNIFORM_W)}>Uniform</button>
            </div>
          </div>
          {HYPS.map((h, i) => (
            <div key={h.id} style={{
              ...
              {/* top row: label + desc + current belief % */}
              <div onClick={() => setTrueIdx(i)} style={{
                display:"flex", alignItems:"center", gap:6,
                padding:"5px 8px 3px", cursor:"pointer",
              }}>
                <span style={S.pill(h.color)}>{h.label}</span>
                <span style={{ flex:1, fontSize:11, color:"#475569" }}>{h.desc}</span>
                <span style={{ fontSize:10, color:"#94a3b8" }}>belief:</span>
                <span style={{ minWidth:36, textAlign:"right", fontWeight:700, color:h.color, fontSize:12 }}>
                  {(weights[i]*100).toFixed(1)}%
                </span>
              </div>
              {/* prior slider row */}
              <div style={{ display:"flex", alignItems:"center", gap:6, padding:"2px 8px 5px" }}
                   onClick={e => e.stopPropagation()}>
                <span style={{ fontSize:10, color:"#94a3b8", minWidth:30 }}>prior</span>
                <input
                  type="range" min={1} max={97} step={1}
                  value={Math.round(priors[i]*100)}
                  onChange={e => setPriorAt(i, parseInt(e.target.value)/100)}
                  style={{ flex:1, accentColor:h.color, height:14 }}
                />
                <span style={{
                  minWidth:36, textAlign:"right", fontWeight:700,
                  color:h.color, fontSize:12,
                }}>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:10, marginBottom:4 }}>
            ...
            {simStale && (
              <span style={{
                ...
                ⚠ priors changed — re-run to update
              </span>
            )}
          </div>
          ...
          <table style={{ width:"100%", borderCollapse:"collapse" }}>
            ...
            <tbody>
              ...
              {(() => {
                ...
                let aqE, aqL, acE, acL;
                if (priorWeighted) {
                  const totalPrior = priors.reduce((s, w) => s + w, 0);
                  aqE = HYPS.reduce((s, h, i) => s + priors[i] * simRes.eig.byHyp[i].nq  / simRes.eig.byHyp[i].n,  0) / totalPrior;
                  aqL = HYPS.reduce((s, h, i) => s + priors[i] * simRes.ellr.byHyp[i].nq / simRes.ellr.byHyp[i].n, 0) / totalPrior;
                  acE = HYPS.reduce((s, h, i) => s + priors[i] * simRes.eig.byHyp[i].ok  / simRes.eig.byHyp[i].n,  0) / totalPrior;
                  acL = HYPS.reduce((s, h, i) => s + priors[i] * simRes.ellr.byHyp[i].ok / simRes.ellr.byHyp[i].n, 0) / totalPrior;
                } else {
                  const re = simRes.eig.total, rl = simRes.ellr.total;
                  aqE = re.nq/re.n; aqL = rl.nq/rl.n;
                  acE = re.ok/re.n; acL = rl.ok/rl.n;
                }
                const eqStr = priorWeighted
                  ? HYPS.map((h, i) => `${h.label}×${priors[i].toFixed(2)}`).join(" + ")
                  : HYPS.map((h)    => `${h.label}×${(1/HYPS.length).toFixed(2)}`).join(" + ");
                const WIN_MARGIN = 0.08;
                const win = aqE < aqL - WIN_MARGIN ? "EIG"
                           : aqL < aqE - WIN_MARGIN ? "E[logLR]"
                           : "Tie";
